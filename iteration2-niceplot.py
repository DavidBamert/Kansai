...
#plottimes: 11 evenly spaced points in time !geht nur mit dt = 0.5 oder 1.0 
plottimes = np.arange(0,TIME+TIME/10,TIME/10)
Rb = np.linspace(0,1,cols)
Gb = 0.4
Bb = np.linspace(1,0,cols)
...
for j in range(0, cols):
    ...
    if any(tt == plottimes):
        plt.plot(A[:],-dzsum, color = [Rb[j],Gb,Bb[j]])
